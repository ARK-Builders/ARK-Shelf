...
class SearchEditFragment : Fragment(R.layout.fragment_search_edit) {
    ...
    private var currentState = SearchEditScreen.SEARCH
    ...
    private fun initUI() = with(binding) {
        ...
        rvLatest.adapter = linkListAdapter.withLoadStateFooter(loaderAdapter)
        rvLatest.loadSkeleton(R.layout.item_link_skeleton) {
            itemCount(16)
        }
        linkListAdapter.addLoadStateListener { loadStates ->
            if (loadStates.refresh is LoadState.NotLoading)
                rvLatest.hideSkeleton()
        }
        lifecycleScope.launch {
            viewModel.listData.collect {
                linkListAdapter.submitData(it)
            }
        }
    ...
    private fun render(state: SearchEditState) = lifecycleScope.launch {
        ...
        if (currentState != state.screen) {
            currentState = state.screen
            linkListAdapter.refresh()
        }
    }
